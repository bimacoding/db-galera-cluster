...
Antarmuka terminal interaktif — **Windows, Linux, macOS**.

```bash
cd tui && cargo build --release
# dari folder mariadb-galera-cluster-fix:
./tui/target/release/galera-tui
```

| Menu TUI | Setara CLI |
|----------|------------|
| Dashboard / Status | `ansible mariadb_cluster` status check |
| Start / Stop Cluster | `ansible-playbook --tags start_cluster/stop_cluster` |
| Deploy / Reset+Deploy | `./run-deploy.sh` / `./run-full-deploy.sh` |
| Apply Config | `./apply-config.sh` |
| Edit Inventory / Tuning | edit `inventory.yml`, `group_vars/all/mariadb.yml` |
| Tambah Node DB | tambah host ke inventory + deploy |

Keyboard: `↑↓` navigasi, `Enter` pilih, `r` refresh dashboard, `q` keluar.

Prasyarat sama: Ansible di PATH, collection terinstall, `secrets.yml` terisi.

Detail build Windows/Linux: [`tui/README.md`](tui/README.md).
