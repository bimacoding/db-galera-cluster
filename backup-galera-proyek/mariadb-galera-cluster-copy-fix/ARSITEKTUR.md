...
> Playbook dan panduan operasional: lihat `STEP.md`, `README.md`, dan `deploy-mariadb-cluster.yml`.
...
| Peran | Dijalankan di |
|-------|----------------|
| `setup-ssh.sh`, `prepare-cluster.sh`, `ansible-playbook` | **Mesin Administrator** (satu mesin — laptop/PC/VM manajemen) |
...
File `inventory.yml` — disesuaikan saat menjalankan `prepare-cluster.sh` atau edit manual:
...
4. **Start app** → test login, API, insert/update
5. **Matikan MySQL lokal** di BM1 & BM2 — **jangan dual-write** ke DB lama
...
| File | Isi |
|------|-----|
| `STEP.md` | Panduan instalasi, operasi, tambah node |
